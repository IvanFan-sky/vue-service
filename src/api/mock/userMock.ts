 ...
 */
const POSITIONS = [
  '前端工程师', '后端工程师', '全栈工程师', '产品经理', '项目经理',
  'UI设计师', 'UX设计师', '运营专员', '市场专员', '销售经理',
  'HR专员', '财务专员', '法务专员', '行政助理', '总监', 'VP'
]
...
export const userMockApi = {
  ...
  getList: async (params: UserListQuery): Promise<UserListResult> => {
    ...
    if (params.startDate) {
      filteredUsers = filteredUsers.filter(user => 
        new Date(user.createdAt) >= new Date(params.startDate!)
      )
    }
    if (params.endDate) {
      filteredUsers = filteredUsers.filter(user => 
        new Date(user.createdAt) <= new Date(params.endDate!)
      )
    }
   ...
   * 批量操作
   */
  batchOperation: async (data: BatchOperationRequest): Promise<{
    successCount: number
    failureCount: number
    message: string
  }> => {
  ...
  getStatistics: async (): Promise<UserStatistics> => {
    ...
    // 状态分布
    const statusDistribution = mockUsers.reduce((acc, user) => {
      acc[user.status] = (acc[user.status] || 0) + 1
      return acc
    }, {} as Record<UserStatus, number>)
    
    // 部门分布
    const departmentDistribution = mockUsers.reduce((acc, user) => {
